...
function takimSkoru() {
  ...
  let takimSkoru = Math.round(Math.random() * 25);
  while (takimSkoru < 10) {
    takimSkoru = Math.round(Math.random() * 25);
  }
  return takimSkoru;
}
...
function macSonucu(skorUret, ceyrek) {
  /*Kodunuzu buraya yazınız*/
  let evSahibi = 0;
  let konukTakim = 0;
...
function periyotSkoru(random) {
  /*Kodunuzu buraya yazınız*/

  let evSahibi = random();
  let konukTakim = random();
  return { EvSahibi: evSahibi, KonukTakim: konukTakim };
}
...
function skorTabelasi(randomPeriyotSkoru, randomTakimSkoru, ceyrek) {
  /*Kodunuzu buraya yazınız*/
  let macSkoru = [];
  let evSahibi = 0;
  let konukTakim = 0;
  ...
  for (let i = 0; i < ceyrek; i++) {
    skorPeriyot = randomPeriyotSkoru(randomTakimSkoru);
    evSahibi += skorPeriyot.EvSahibi;
    konukTakim += skorPeriyot.KonukTakim;
    macSkoru.push(
      `${i + 1}. Periyot: Ev sahibi ${skorPeriyot.EvSahibi} - Konuk Takım ${
        skorPeriyot.KonukTakim
      } `
    );
  }
  ...
  while (evSahibi === konukTakim) {
    ...
    konukTakim += skorPeriyot.KonukTakim;
    macSkoru.push(
      `${i + 1}. UZATMA: Ev sahibi ${skorPeriyot.EvSahibi} - Konuk Takım ${
        skorPeriyot.KonukTakim
      } `
    );
    i++;
  ...
  macSkoru.push(
    `Mac Sonucu: Ev Sahibi: ${evSahibi} - Konuk Takim: ${konukTakim}`
  );
